...
class HackPesel {
	class Pesel {  // klasa wewnętrzna, "ukryta" przed światem
		...
		Pesel(String p){ // konstruktor, bierze napis i przypisuje go do pól
			...
			for(int i=0;i<len;i++) val[i] = Integer.valueOf(str.charAt(i))-48; // i przepisuje do niej cyfry
			//dłuższe ale robi to samo:
			//for(int i=0;i<len;i++) val[i] = Character.getNumericValue(str.charAt(i));
		}
  ...
  HackPesel(String p){     // konstruktor głównej klasy
	...
	void print(){ System.out.println(pesel.toStr()); }  // wypisanie pesel
  void print(String s){ System.out.println(s); }      // wypisanie czegokolwiek
